...
## Purpose
TBD - created by archiving change rename-to-scdice-metrics. Update Purpose after archive.
## Requirements
...
### Requirement: Old Package Identity Removal
The package SHALL remove all references to the old package name `scdcb-metrics` (Python package name: `scdcb_metrics`) during the rename process.

#### Scenario: Package name cleanup
- **WHEN** the rename is complete
- **THEN** no configuration files contain references to `scdcb-metrics` or `scdcb_metrics`

#### Scenario: Migration guidance
- **WHEN** users encounter references to the old package name
- **THEN** they should update all references to `scdice-metrics` or `scdice_metrics` respectively
